...
{
	player = App->entities->GetPlayerInfo(); // we need to get player info from the awake ! (only called in a module!)
	...
	collider = App->collision->AddCollider({ (int)position.x, (int)position.y, PLAYER_SIZE_X, PLAYER_SIZE_Y }, COLLIDER_PLAYER, App->entities);
...
void Player::Update(float dt)
{
	if (player.god_mode == false)
	{
		...
		if (going_left)
		{
			...
			iPoint worldPos = App->map->WorldToMap(position.x - player.velocity_x - 1, position.y);
		...
		if (going_right)
		{
			...
			// (x + w, y + h) point where the player's ending coordinates are located in the world
			iPoint worldFinalPos = App->map->WorldToMap(position.x + PLAYER_SIZE_X + player.velocity_x + 1, position.y + PLAYER_SIZE_Y - 1);
		...
		if (up_and_down && !dash)
		{
			...
			iPoint worldNextPos = App->map->WorldToMap(position.x, previous_position.y + initial_speed * (time + 0.1f) + (player.gravity*(time + 0.1f)*(time + 0.1f)) * 0.5f + 1);
			// (x + w, y + h) point where the player's ending coordinates are located in the world
			iPoint worldFinalPos = App->map->WorldToMap(position.x + PLAYER_SIZE_X - player.velocity_x - 1, position.y + PLAYER_SIZE_Y);
			// (x + w, y + h) point where the player's ending coordinates are located in the world in the next frame
			iPoint worldNextFinalPos = App->map->WorldToMap(position.x + PLAYER_SIZE_X - player.velocity_x - 1, previous_position.y + initial_speed * (time + 0.1f) + (player.gravity*(time + 0.1f)*(time + 0.1f)) * 0.5f + PLAYER_SIZE_Y);
		...
		if (!dash)
		{
			if (equation_is_possible == 0) position.y = previous_position.y + initial_speed * time + (player.gravity*time*time) * 0.5f;
